...
public class Entrada {
  ...
  public static void salvarCursos(String nomeDoArquivo) throws FileNotFoundException, IOException, InputMismatchException, Exception {
  ...
  public static void salvarDisciplinas(String nomeDoArquivo) throws FileNotFoundException, IOException, InputMismatchException, Exception {
      // Abre o arquivo de entrada
      FileInputStream fluxoDeEntrada = new FileInputStream(nomeDoArquivo);
...
public static void salvarAvaliacoes(String nomeDoArquivo) throws FileNotFoundException, IOException, InputMismatchException, Exception, DateTimeParseException {
    // Abre o arquivo de entrada
    FileInputStream fluxoDeEntrada = new FileInputStream(nomeDoArquivo);
    ...
    while ((linha = conteudoCSV.readLine()) != null) {
        ...
        int pesoDaAvaliacaoINT = Integer.parseInt(informacaoDaAvaliacao[3]); //convertendo para inteiro para imprimir assim

        // Verifica se o peso da avaliação é válido
        if (pesoDaAvaliacao <= 0) {
            conteudoCSV.close();
            
            throw new Exception("Peso de avaliação inválido para " + codigoDaAvaliacao + ": " + pesoDaAvaliacaoINT + ".");
        } else {
            ...
            if (mapaDeAvaliacoes.containsKey(codigoDaAvaliacao)) {
                ...
                if (tipoDaAvaliacao.equalsIgnoreCase("P")) {
                    ...
                    if (mapaDeDisciplinas.containsKey(codigoDaDisciplina)) {
                        ...
                        Prova prova = new Prova(mapaDeDisciplinas.get(codigoDaDisciplina), codigoDaAvaliacao, nomeDaAvaliacao, pesoDaAvaliacao, dataDaAvaliacao, valorDeData);
                        ...
                        conteudoCSV.close();
                        throw new Exception("Código de disciplina não definido usado na avaliação " + codigoDaAvaliacao + ": " + codigoDaDisciplina + ".");
                    }
                    ...
                    if (informacaoDaAvaliacao.length == 7) {
                        conteudoCSV.close();
                        tamanhoDoGrupoDaAvaliacao = Integer.parseInt(informacaoDaAvaliacao[6]);
                        throw new Exception("Tamanho máximo de grupo especificado para a prova " + codigoDaAvaliacao + ": " + tamanhoDoGrupoDaAvaliacao + ".");
                    }
                    ...
                    if (mapaDeDisciplinas.containsKey(codigoDaDisciplina)) {
                        ...
                        if (tamanhoDoGrupoDaAvaliacao <= 0) {
                            conteudoCSV.close();
                            throw new Exception("Tamanho máximo de grupo inválido para trabalho " + codigoDaAvaliacao + ": " + tamanhoDoGrupoDaAvaliacao+".");
                        } else {
                            // Cria um objeto Trabalho
                            Trabalho trabalho = new Trabalho(mapaDeDisciplinas.get(codigoDaDisciplina), codigoDaAvaliacao, nomeDaAvaliacao, pesoDaAvaliacao, dataDaAvaliacao, valorDeData, tamanhoDoGrupoDaAvaliacao);
                        ...
                        conteudoCSV.close();
                        throw new Exception("Código de disciplina não definido usado na prova " + codigoDaAvaliacao + ": " + codigoDaDisciplina + ".");
                    }
                } else if (tipoDaAvaliacao.equalsIgnoreCase("F")) {
                    // Verifica se o código da disciplina existe no mapa de disciplinas
                    if (mapaDeDisciplinas.containsKey(codigoDaDisciplina)) {
                        // Cria um objeto ProvaFinal
                        ProvaFinal provaFinal = new ProvaFinal(mapaDeDisciplinas.get(codigoDaDisciplina), codigoDaAvaliacao, nomeDaAvaliacao, pesoDaAvaliacao, dataDaAvaliacao, valorDeData, tipoDaAvaliacao);
                    ...
                    conteudoCSV.close();
                    throw new Exception("Tipo de avaliação desconhecido para " + codigoDaAvaliacao + ": " + tipoDaAvaliacao + ".");
                }
            }
        }
    }
...
public static void salvarAlunos(String nomeDoArquivo) throws FileNotFoundException, IOException, InputMismatchException, Exception {
        ...
        if(mapaDeAlunos.containsKey(matriculaDoAluno.toString())){
            ...
            if (tipoDeAluno.equalsIgnoreCase("G")) {
                ...
                if (mapaDeCursos.containsKey(codigoDoCurso)) {
                    ...
                    throw new Exception("Código de curso não definido usado no aluno " + matriculaDoAluno + ": " + codigoDoCurso + ".");
                }
                ...
                if (codigoDoCurso.equalsIgnoreCase("M")){
                    ...
                    throw new Exception("Tipo de aluno de pós-graduação desconhecido para " + matriculaDoAluno + ": " + codigoDoCurso + ".");
                ...
                throw new Exception("Tipo de aluno desconhecido para " + matriculaDoAluno + ": " + tipoDeAluno + ".");
            ...
            for (int i = 0; i < codigosDasDisciplinas.length; i++) {
                ...
                if(mapaDeDisciplinas.containsKey(codigosDasDisciplinas[i])){
                    ...
                    mapaDeDisciplinas.get(codigosDasDisciplinas[i]).adicionaAluno(matriculaDoAluno.toString(), mapaDeAlunos.get(matriculaDoAluno.toString()));

                    // Verifica se o curso da disciplina já existe no mapa de cursos da disciplina
                    if(mapaDeDisciplinas.get(codigosDasDisciplinas[i]).retornaMapaDeCursos().get(codigoDoCurso) == null){

                        // Adiciona o curso ao mapa de cursos da disciplina
                        mapaDeDisciplinas.get(codigosDasDisciplinas[i]).retornaMapaDeCursos().put(codigoDoCurso , mapaDeCursos.get(codigoDoCurso));
                    
                    }

                    // Verifica se a disciplina já existe no mapa de disciplinas do curso
                    if(mapaDeCursos.get(codigoDoCurso).retornaMapaDeDisciplinas().get(codigosDasDisciplinas[i]) == null){

                        // Adiciona a disciplina ao mapa de disciplinas do curso
                        mapaDeCursos.get(codigoDoCurso).retornaMapaDeDisciplinas().put(codigosDasDisciplinas[i],mapaDeDisciplinas.get(codigosDasDisciplinas[i]));
                  ...
                  throw new Exception("Código de disciplina não definido usado " + codigosDasDisciplinas[i] + ".");
  ...
  public static void salvarNotas(String nomeDoArquivo) throws FileNotFoundException, IOException, InputMismatchException, Exception {
          ...
          if (mapaDeAvaliacoes.containsKey(codigoDaAvaliacao)) {
              ...
              if (notaDoAluno > 10 || notaDoAluno < 0) {
                ...
                notaDoAlunoFormatada = notaDoAlunoFormatada.replace(".",",");
                  ...
                  throw new Exception("Nota inválida para avaliação " + codigoDaAvaliacao + " do(s) aluno(s) " + matriculas + ": " + notaDoAlunoFormatada + ".");
                  ...
                  for (int i = 0; i < matriculaDosAlunos.length; i++) {
                      ...
                      if (mapaDeAlunos.containsKey(matriculaDoAluno)) {
                          ...
                          if (mapaDeAvaliacoes.get(codigoDaAvaliacao).getDisciplinaDaAvaliacao().retornaMapaDeAlunos().get(matriculaDoAluno) == null) {
  
                              conteudoCSV.close();
  
                              // Lança uma exceção informando que o aluno não está matriculado na disciplina
                              throw new Exception("O aluno " + matriculaDoAluno + " possui nota na avaliação " + codigoDaAvaliacao + " da disciplina " + mapaDeAvaliacoes.get(codigoDaAvaliacao).getDisciplinaDaAvaliacao().getCodigoDaDisciplina() + ", porém não encontra-se matriculado nesta disciplina.");
  
                          }
  
                          // Verifica se o aluno já possui nota registrada para a avaliação
                          if (mapaDeAvaliacoes.get(codigoDaAvaliacao).retornaMapaDeNotas().get(matriculaDoAluno) != null) {
  
                              conteudoCSV.close();
  
                              // Lança uma exceção informando que o aluno foi registrado em mais de um grupo para a avaliação
                              throw new Exception("O aluno " + matriculaDoAluno + " foi registrado em mais de um grupo para a avaliação " + codigoDaAvaliacao + ".");
                          ...
                          if (mapaDeAvaliacoes.get(codigoDaAvaliacao).getTipoDaAvaliacao().equals("T")) {
                              ...
                              double pesoTrabalho = (mapaDeAvaliacoes.get(codigoDaAvaliacao).getPesoDaAvaliacao()) / (matriculaDosAlunos.length);
                          ...
                          throw new Exception("Matrícula de aluno não definida usada na planilha de notas, associada à avaliação " + codigoDaAvaliacao + ": " + matriculaDoAluno + ".");
                  ...
                  Integer quantidadeDeAlunosMaxima = mapaDeAvaliacoes.get(codigoDaAvaliacao).retornaQuantidadeDeAlunos();
  
                  // Verifica se a quantidade de alunos na planilha é maior do que a quantidade máxima permitida
                  if (matriculaDosAlunos.length > quantidadeDeAlunosMaxima) {
  
                      conteudoCSV.close();
  
                      // Lança uma exceção informando que a quantidade de alunos é inválida
                      throw new Exception("Quantidade de alunos inválida para " + codigoDaAvaliacao + ", quantidade máxima: " + quantidadeDeAlunosMaxima + ".");
              ...
              throw new Exception("Código de avaliação não definido usado na planilha de notas, associado ao(s) aluno(s) " + matriculas + ": " + codigoDaAvaliacao + ".");
